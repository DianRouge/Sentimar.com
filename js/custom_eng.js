    ...
    if($('#home').length){
      ...
      //  $('#home').height(570);
        $.supersized({
                    ...
                    {image : './images/slider/super/supersized-1.jpg', title : '<h1 class="bigTitle">EQUIPMENT AND SYSTEMS<br /> <small>FOR MARITIME AND PORT APPLICATIONS</small></h1>', thumb : '', url : ''}
                    ],
...
/*===================
    ...
    var recaptcha_response_field = $('#recaptcha_response_field').val();
        var dataString = 'name=' + name + '&email=' + email + '&subject=' + subject + '&message=' + message+'&recaptcha_response_field='+recaptcha_response_field+'&recaptcha_challenge_field='+recaptcha_challenge_field;
    
    
    if (!IsEmail(email)) {
        //code
        $('#valid-issue').html('Por favor escribe un email valido').show();
        
    }
    else{
        if (name === '' || !IsEmail(email) || subject === '' || message === '') {
            $('#valid-issue').html('Por Favor llena todos los campos').show();
        } else {
        ...
        $.ajax({
            ...
            if(data=='captcha incorrecto'){
                ...
                $('#contactform').hide();
                $('#valid-issue').html('Tu mensaje ha sido enviado, nos comunicaremos pronto').show();
            }
            },
            error: function(data) {
            alert(data); // apple
    ...
    $("#contactform2").submit(function () {
      /*  var name = $("#name").val();
        ...
        if (name === '' || !IsEmail(email) || subject === '' || message === '') {
            $('#valid-issue').html('Please, complete the form').show();
        } else {
            $.ajax({
                type: "POST",
                url: "../assets/php/submit.php",
                ...
                success: function () {
                    $('#contactform2').hide();
                    $('#valid-issue').html('Your message has been sent!').show();
                }
    ...
    if (!IsEmail(email)) {
        //code
        $('#valid-issue').html('Please input a valid email').show();
